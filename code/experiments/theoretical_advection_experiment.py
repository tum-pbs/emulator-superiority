...
def get_data(
    *,
    ...
    config = {
        ...
        "temporal_horizon_train": num_unrolled_steps,
        "temporal_horizon_test": 100,
        "num_warmup_steps": 0,
        "metric_fn_train": "mean_MSE",
        "metric_fn_test": "mean_RMSE",
        "num_samples_train": 300,
        "num_samples_test": 10,
        "num_unrolled_steps": num_unrolled_steps,
        # "optimizer": "adam;10_000;warmup_cosine;0.0;1e-3;1_000",
        "optimizer": "adam;10_000;constant;1e-2",  # The optimization problem is convex
        "batch_size": 32,
        "seed_init": seed_init,
...
def get_all_data(
    ...
    os.makedirs("theoretical_advection_data", exist_ok=True)
    data_metrics = []
    data_loss = []
    for setting in tqdm(settings):
        setting_str = json.dumps(setting)
        if os.path.exists(f"theoretical_advection_data/{setting_str}"):
            data_metrics.append(
                pd.read_csv(f"theoretical_advection_data/{setting_str}/metrics.csv.gz", compression="gzip")
            )
            data_loss.append(
                pd.read_csv(f"theoretical_advection_data/{setting_str}/loss.csv.gz", compression="gzip")
            )
        else:
            os.makedirs(f"theoretical_advection_data/{setting_str}", exist_ok=True)
            dm, dl = get_data(**setting)
            # Compress the data
            dm.to_csv(
                f"theoretical_advection_data/{setting_str}/metrics.csv.gz", index=False, compression="gzip"
            )
            dl.to_csv(
                f"theoretical_advection_data/{setting_str}/loss.csv.gz", index=False, compression="gzip"
            )
